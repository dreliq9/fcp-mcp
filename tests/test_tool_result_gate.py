...
def test_tool_result_gate_script_checks_the_full_catalog():
    ...
    assert result.stdout == "94 tools checked; 0 legacy result schemas\n"
...
def test_gate_rejects_catalog_count_other_than_94():
    from scripts.check_tool_results import failures
    ...
    assert asyncio.run(failures(FakeServer())) == [
        "catalog: expected 94 tools, got 93"
    ]
...
def test_script_main_returns_nonzero_for_catalog_or_schema_failure(
    ...
    async def fake_check():
        return 93, ["catalog: expected 94 tools, got 93"]

    monkeypatch.setattr(gate, "_check", fake_check)

    assert gate.main() == 1
    assert capsys.readouterr().out == "catalog: expected 94 tools, got 93\n"
