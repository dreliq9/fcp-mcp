...
def test_gate_rejects_catalog_count_other_than_89():
    from scripts.check_tool_results import failures
    ...
    assert asyncio.run(failures(FakeServer())) == [
        "catalog: expected 89 tools, got 88"
    ]
...
def test_script_main_returns_nonzero_for_catalog_or_schema_failure(
    ...
    async def fake_check():
        return 88, ["catalog: expected 89 tools, got 88"]

    monkeypatch.setattr(gate, "_check", fake_check)

    assert gate.main() == 1
    assert capsys.readouterr().out == "catalog: expected 89 tools, got 88\n"
