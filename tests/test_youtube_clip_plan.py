...
from fcp_mcp.youtube_clip_plan import MATERIALIZED_SCHEMA, PROVENANCE_SCHEMA, register_youtube_clip_plan_tool
...
def _runtime(tmp_path: Path):
    ...
    def resolve_output(output_path: str, *, input_path=None, default_name=None, suffixes=()) -> Path:
    ...
    def artifact_for_path(path: Path, *, media_type: str) -> ArtifactReference:
        resolved = Path(path).resolve()
        return ArtifactReference(path=str(resolved), media_type=media_type, sha256=_sha256(resolved), size_bytes=resolved.stat().st_size)

    def artifact_for_receipt(receipt) -> ArtifactReference:
        return artifact_for_path(receipt.destination, media_type="application/vnd.apple.fcpxml+xml")

    def receipt_result(receipt) -> TransactionReceiptResult:
        return TransactionReceiptResult(transaction_id=receipt.transaction_id, source=None, destination=str(receipt.destination), backup_path=str(receipt.backup_path) if receipt.backup_path else None, input_sha256=None, prior_sha256=receipt.prior_sha256, output_sha256=receipt.output_sha256, validation_warnings=list(receipt.validation_warnings), elapsed_ms=receipt.elapsed_ms, disposition="committed")

    runtime = SimpleNamespace(TOOLS=ToolRegistry(), CONFIG=SimpleNamespace(log_format="text"), FCPXMLGenerator=FCPXMLGenerator, atomic_replace_bytes=atomic_replace_bytes, _resolve_input=resolve_input, _resolve_output=resolve_output, _artifact_reference=artifact_for_receipt, _artifact_reference_for_path=artifact_for_path, _receipt_result=receipt_result)
    register_youtube_clip_plan_tool(runtime)
    return runtime


def _manifest(tmp_path: Path) -> Path:
    first, second = tmp_path / "clip-a.mp4", tmp_path / "clip-b.mp4"
    first.write_bytes(b"first-video")
    second.write_bytes(b"second-video")
    payload = {"schema": MATERIALIZED_SCHEMA, "plan_revision": "cp-aaaaaaaaaaaaaaaaaaaaaaaa", "materialization_revision": "cm-bbbbbbbbbbbbbbbbbbbbbbbb", "assets": [
        {"clip_id": "clip-001", "video_id": "jNQXAC9IVRw", "title": "History", "channel": "Historian", "source_url": "https://www.youtube.com/watch?v=jNQXAC9IVRw", "source_start_s": 98.0, "source_end_s": 108.0, "duration_s": 10.0, "path": str(first), "sha256": _sha256(first)},
        {"clip_id": "clip-002", "video_id": "dQw4w9WgXcQ", "title": "Artist", "channel": "Studio", "source_url": "https://www.youtube.com/watch?v=dQw4w9WgXcQ", "source_start_s": 40.0, "source_end_s": 46.0, "duration_s": 6.0, "path": str(second), "sha256": _sha256(second)},
    ]}
...
def test_handoff_generates_native_fcpxml_and_provenance(tmp_path: Path) -> None:
    ...
    result = handler(str(_manifest(tmp_path)), project_name="Frontier Montage", output_path=str(tmp_path / "frontier.fcpxml"))
    assert result.selected_clip_count == 2
    assert result.target_duration_seconds == 16.0
    assert result.hashes_verified is True
    document = FCPXMLParser().parse(Path(result.destination.path))
    assert document.projects[0].name == "Frontier Montage"
    assert [clip.name for clip in document.projects[0].clips] == ["History", "Artist"]
